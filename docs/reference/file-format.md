...
.tgscene版2はcomponentsに `{ "role": "terrain" または "cloud", "asset": { "uid": 固定ID, "path": ルート相対パス } }` を保存する。グラフと資源参照表は.tgterrain / .tgcloudへ分離し、天球とカメラ・見え方はシーンが参照・保持する。旧版1も継続して読み書きする。詳細は[コンポーネント分離](../design/project-workspace.md)を参照。

グラフ内のIDは各ファイル内で独立。実行時のcomponentフィールドは編集領域の識別用で、アセット内には保存しない。ペイント参照はsourceの固定IDで解決する。
...
種類名modelMerge（内部番号48）。追加パラメータは持たず、可変長のinputsとInstancesのoutputsを保存する。読み込み時に全入力IDを復元してからリンクを解決し、既存の接続IDを保って空き入力を末尾の1つへ整理する。
...
レイヤーの`crumbling.avoidPointOverlap`（bool）にPointsの重なり回避を保存する。新規ノードの既定はtrue。キーのない既存ファイルはfalseで読み、従来の配置を保つ。件数・空間ハッシュ・GPU読み戻し状態は保存しない。
...
- 画像が見つからない → **警告を出して読み込みを続け、参照は「リンク切れ」として残す。**
  テクスチャライブラリにパスと名前だけの項目（`LibraryTexture::missing`）を作り、
  マテリアルやノードの割り当てはそこへ繋いだままにする。合成では「なし」と同じに
  扱われる（SRV インデックスが無効値になる）。**保存すると元のパスがそのまま書き出される**
  ので、別の環境で開いて保存しても参照は失われない。
  繋ぎ直しはテクスチャ一覧の右クリック（ファイル指定 / 元の場所から読み直す /
  フォルダ指定でまとめて）から行い、ID を保ったまま中身だけが入る。
- 値の型が食い違っている → その項目だけ既定値に落とす。
- レイヤーが 1 枚も無い → 下地を 1 枚だけ置く（空のスタックは操作の起点が無い）。

## モデルアセット

`models[]` は任意の追加キー。旧ファイルで省略されていれば空一覧となる。各要素は `name`、プロジェクト基準の相対 `path`（FBX）、`materials[]` を持つ。マテリアル参照は同じプロジェクトの `materials[].id`、未割り当てはnull。元のFBXはコピーせず参照する。

FBXからLODGroupの各段階・UV0・法線・マテリアルスロットを再構築する。単位はm、軸は右手系Y-upへ変換し、ノード階層・ジオメトリ変換を頂点へ反映する。UVはVを反転してDirectXの画像座標へ合わせる。寸法は全LODの包囲箱。元ファイルが無い場合も名前・パス・割り当ては保存し直せる。自動生成LODや配置インスタンス、スキニングはこの形式に含まない。

依存はvcpkg manifestとローカルoverlay port `ports/ufbx` でufbx 0.17.1へ固定し、配布アーカイブのSHA512を検証する。座標変換の根拠: [ufbxの座標系仕様](https://ufbx.github.io/elements/nodes/#coordinate-spaces)。

検証用起動引数は `--import-model <FBX>`、`--preview-model <0始まりのモデル番号>`、`--preview-model-lod <LOD番号>`。`--screenshot-ui` または `--save-project` と組み合わせて表示・保存往復を検証できる。
