...
更新日時: 2026-09-25 16:34

## LLM がアプリを動かしてノードを組める仕組み（2026-09-25 16:34、予定・未着手）

LLM がアプリ（EXE 化した後も）を実行し、シーンを読み、ノードやパスを組み、結果を確かめるところまでを自分で回せるようにする。天狗岳の登山道づくり（2026-09-25）では、JSON を直接書き換え、緯度経度 → UV を手で計算し、経路の再計算はユーザーに押してもらい、結果はスクリーンショットで目視するしかなかった。アプリが開いていると書き換えが衝突するため、毎回閉じてもらう必要もあった。

### 方針

- **UI と同じ処理を通す。** 操作は NodeGraph / Path の既存の関数（`CreateLink` / `CheckLink` / `PastePathClip` など）を呼ぶだけにして、JSON を外から直接いじる経路を作らない。検証（循環・型・範囲）も UI と同じになる。
- **決まった操作だけを受ける。** 任意のコードは実行しない。操作は名前と引数の組で、知らない操作や範囲外の値は失敗として返す。
- **入出力は JSON。** 結果（作ったノードの ID、失敗の理由、警告）を機械が読める形で返す。ログの文字列を読ませない。
- **失敗は途中で止めて何も変えない。** 1 つの操作列はまとめて成功するか、何も変えないかのどちらか。書き換える前にバックアップを取る。
- EXE と一緒に配る説明書だけで使えること（リポジトリを読まなくてよい）。

### 段階

1. **ノードの目録と検査（`--describe-nodes` / `--inspect`）**
   - `--describe-nodes <out.json>`: ノードの種類ごとに、ピン（名前・型・入出力）、パラメータ（名前・型・範囲・既定値・説明）を書き出して終了する。LLM がノードを組むときの辞書になる。定義テーブルとプロパティの `Property*` の範囲から作り、手書きの一覧は持たない（食い違いを防ぐ）。
   - `--inspect <scene> <out.json>`: シーンを開いて評価まで待ち、グラフ（ノード・リンク・ピンの型）、循環、読み込みで捨てたリンク、切れたアセット参照、評価のエラー、パスの点・エッジ・鎖の数、地形の実寸と位置情報を書き出して終了する。
2. **地形の位置情報と地理データの取り込み**
   - 地形（`.tgterrain`）に位置情報（緯度経度の範囲、標高の最小・最大、出どころ）を持たせ、プレビュー設定で見せる。ハイトマップを作ったツール（location-viewer）の範囲をそのまま入れられる。
   - Path ノードに「地理データから取り込む」を足す。GeoJSON と location-viewer の workspace.json の線を、位置情報で UV へ変換し、マップの端で切り、分岐をつなぎ、形を保って間引く（天狗岳で `build_trails.py` がやったことをアプリの機能にする）。
3. **操作の実行（`--apply <ops.json> [--save]`）**
   - 操作列を順に実行する。最初に用意する操作: ノードの作成・削除、パラメータの設定、接続・切断、パスの点とエッジの追加、地理データの取り込み、経路の再計算、保存、スクリーンショット。
   - 作ったノードは操作列の中で付けた名前（キー）で後から参照できる（ID を先に知らなくてよい）。
   - 結果の JSON に、作ったノードの ID、失敗した操作と理由、警告（循環を断った、範囲で丸めた等）を書く。
   - 確認の例: 天狗岳の登山道を、この操作列だけで作り直せること。
4. **説明書**
   - `docs/reference/` に、ファイル形式（シーン・地形・雲・パス）と操作の JSON の仕様、使い方の例（地形を読む → マスクを組む → 登山道を取り込む → 確かめる）をまとめる。JSON Schema も付け、EXE の隣に置いて配る。
5. **起動中のアプリを操作する口（MCP）**
   - 起動中のアプリへ、名前付きパイプ経由で 3 の操作・1 の検査・スクリーンショットを頼めるようにし、それを MCP サーバとして LLM に見せる。フレームの合間にメインスレッドで実行し、アンドゥの履歴に積み、未保存として扱う（ユーザーの編集と衝突しない）。
   - 既定はオフ。設定で有効にしたときだけ口を開き、ローカルからだけ受ける。

1 と 2 を先に作る（小さく、今回の手間の大半がなくなる）。3 は 1 の目録ができてから、5 は 3 の操作がそろってから。

## 植生の色むらと季節（2026-09-25 00:56、1〜3 実装済み）

株の色の違いを、局所的な対処（バリエーションごとにマテリアルを分けるなど）ではなく、次の 3 つに分けて持つ。

- **どこで（グラフ）**: 配置の点に汎用の属性チャンネル（4 つ）を持たせ、点の元がマスクの値を書く。グラフは場所の性質（湿り気、標高、斜面の向き、ノイズ）だけを渡し、色は知らない。
- **何色に（樹種のマテリアル）**: 属性への応え方をマテリアルが持つ。葉だけが変わり幹は変わらない。同じマスクでも樹種ごとに違う色へ寄せられる。テクスチャの明暗は残して色味だけを寄せる。
- **いつ（シーン）**: 季節をシーンの環境設定として 1 つ持ち、紅葉の進みは「季節 − 点ごとの遅れ」で決める。遅れは属性（標高・北向きほど早い）と株ごとの乱数から作る。

インポスターは色むらを焼かず、画素ごとの「受ける割合」だけを焼いて描画時に掛ける（季節や設定を変えても焼き直さない）。

1. 点の属性チャンネルと Scatter の Variation 入力（済）
2. マテリアルの「属性 → 色」の応え方。メッシュとインポスターで共通の関数（済）
3. 夏山の色むらで確かめる（実装済み、見え方は未確認）
4. 季節の値、樹種ごとの紅葉の色、遅れの計算（属性の y を遅れに使う想定）。落葉はアルファのしきい値を動かせば同じ枠組みに乗る

## 未読み込みアセットの割り当て

ユーザー指定により、素材・テクスチャの候補はプロジェクトルートから列挙し、選択・ドロップ時だけ読み込む方式へ変更する。
全アセットのGPU先読みは行わない。Surface、レイヤー内部、モデルの素材、各テクスチャ欄で共通の選択ヘルパーを使う。

## レイヤーマテリアルの移植（2026-09-20）

Road 移植に先立ち、`.tglayer` を Surface でも使える共有材質として実装する。
ユーザー指定により素材内部はレイヤー操作へ変更する。地形グラフと分離し、地形側のレイヤースタックは復活させない。
第1段階は最大4層、通常 PBR 素材の合成、汎用マスク、保存と再利用。
道路座標を必要とするマスク、Road Path、道路メッシュは後続。実装は [layer-material.md](../design/layer-material.md)。

## オーバーハング編集（検討アイデア・採用未定）

ユーザー依頼により、ハイトマップの崖を局所的な Cliff Patch に置き換える案を
[オーバーハング編集のアイデア](../reference/overhang-editing.md) に記録した。
岩棚・えぐれの表現、地形との接続、積雪との組み合わせ、最小試作と外部出力の課題を整理している。
実装着手や優先順位は未決定で、既存の実装予定は変更しない。

## 夜空の表示と月明かり

ユーザー指定により、手動設定の月・月相・星空と地形・雲への月明かりを追加する。初期対応は昼夜で主光源と影を共有し、月による大気散乱を環境光へ含める。星は星表（BSC5）による実在配置で、観測緯度と恒星時回転で向きを決める。「観測地と日時」で手動と日時計算を切り替え、日時モードでは太陽・月・月相・恒星時を緯度経度と日時から求める。昼間の月、太陽と月の同時直射照明、日時の自動進行は後続。露出は既存の物理カメラ設定を使う。詳細と近似の制約は [大気散乱スカイの設計](../design/atmospheric-sky.md) を参照。雲の位置ごとの拡散環境光は下記の独立した予定として維持する。

## 雲と地形の位置関係を反映する環境光（予定・未着手）

雲量を増やすと雲より上の地形の陰まで白っぽく明るくなる問題に対応する。現在は原点から見た雲入り環境マップを全地形で共有し、拡散環境光の参照に地形の位置を使っていない。ユーザー指定により、位置ごとの環境光評価を今後の実装予定に追加する。

まず拡散環境光を対象に、青空の環境光と雲による遮蔽・散乱光を分ける。地形を覆う粗い3D格子の照明プローブで方向別の雲の透過率・散乱光を評価し、地形の位置と面の向きに応じて補間する方式を基本案とする。格子密度、方向表現、更新頻度、GPU負荷は設計・計測で決める。地形による遮蔽と、補間による山越しの光漏れも検討・検証に含める。雲より上で一律に影響をゼロにせず、下の雲が見える斜面への照り返しは残す。光沢反射の位置依存は後続とし、太陽の直接光に対する雲影とは別に扱う。

検証では露出と太陽・大気・材質を固定し、雲の上・中・下の平面と斜面で雲量だけを変更して比較する。雲より上の上向き面に頭上の雲として照明が入らないこと、雲が見える斜面の照り返し、雲の下の天空遮蔽、山に遮られた雲からの光漏れ、移動・編集時のちらつきと更新負荷を確認する。調査時点ではコードの経路確認のみで、実シーンの比較撮影は未実施。

## 稜線の雪煙（帯メッシュ方式。2026-09-19 に方針変更）

稜線で舞う雪煙は、**稜線から風下へ伸びる半透明の帯を描く方式**で進める。2026-09-18 に決めた
「局所ボリュームのシミュレーションを記録して雲のレイマーチで再生する」計画は、ユーザー判断により
2026-09-19 に取り下げた（稜線から剥がれる細い構造はボリュームの解像度では出しにくく、規模も大きいため）。
すでに作った Wind Field（地形全体の風の場、Spindrift マスク）は発生源と風向きとしてそのまま使う。

第 1 段階（済）: Snow Plume ノード。Spindrift から帯を生やし、周期ノイズで流す。詳細は
[nodes.md](../reference/nodes.md) の Snow Plume。

今後の候補（順不同。見た目を見てから決める）:

- **雲との前後。** いまは雲の手前に常に乗る。雲の半解像度の透過率と平均距離を使って、雲の奥の雪煙を減衰させる。
- **空中遠近。** 大気の合成の後に描くので霞が掛からない。遠景で浮いて見えるなら、大気の LUT で透過と散乱を掛ける。
- **風の場に沿わせる。** 中心線を一様な風ではなく Wind 出力（3D の速度場）の流線でたどる。
- **近景のパーティクル。** カメラの近くだけ細かい飛雪を足す（まずは不要という判断）。
- 風速で長さと発生量を変える、帯どうしの前後の並べ替え。
...
モデルアセットはFBX読み込み、メッシュプレビュー、既存LOD選択、マテリアルスロット参照、保存・履歴まで対応。Crumbling Points → Model Scatter → Model Outputによるインスタンス配置にも対応。GPUの視錐台・距離カリングと描画対象の圧縮、間接描画まで対応。ScatterのPointsによる汎用のポイント散布にも対応。Model Mergeで複数の配置を1つのModel Outputへまとめられる。マテリアルのアルファ抜きと両面表示に対応し、葉のカードを使う植生モデル（這い松）を置ける。距離による LOD の自動切り替え（ディザで重ねる移行つき）と、FBX の `_LODn` 命名の読み込みにも対応。LOD の最終段より遠くは、描画距離で切って地形のマテリアルへ引き継ぐ方針。インポスター（半球 / 全球のオクタヘドラル）は段階 1（焼き込み・保存・削除・モデルプレビューでの表示）、段階 2（Model Scatter の最終段として描く。ディザでの切り替え、Stats、LOD の色分け表示）、段階 3（影の受けと落とし）まで実装済み。次は影の負荷の軽減と遮蔽カリング。影は配置した株を影の段ごとに描き直しており、確認用シーン（3 段）で自動 LOD 時に 8.2 ms、全 LOD0 で 18.3 ms を占める（インポスター中心なら 1.7 ms）。まず PIX で段ごとの内訳を測り、(1) 影用の LOD を本描画より粗くする（影の段では 1〜2 段粗い LOD かインポスターを使う倍率。UE の shadow LOD に相当）、(2) 影の段ごとに距離の範囲で株を絞る、(3) 一定距離より遠い株は影を落とさない、の順で入れる。足りなければ (4) 遠い影の段の使い回し（光源・カメラが大きく動いたときだけ描き直す）を検討する。distance field shadow は遠景の大量の株に有利だが、薄い葉のカードを距離場で表しにくく、背の低い植生では遠くの影が見えにくいので、今は採らない。自動LOD生成は後回しとする。
