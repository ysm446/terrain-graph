...
### 堆積の解析グリッドは合成 Height をエイリアスして読む

`CsSetup` は合成 Height（2048²）を解析グリッド（既定 512²）の中心で
`SampleLevel(..., 0)` する。**平均ではなく間引き**なので、合成 Height に入っている
材質スケールの凹凸（下地サーフェスのハイトマップ）がエイリアスして入る。

- 実測: 解析グリッドを 2048 に上げると、マスクの隣接画素の差が 1.78 → 3.26 と
  **増える**（粗いほうが細部を失うのではなく、別の模様になっている）。
  下地の `起伏の強さ` を 0 にすると 0.56 まで落ちるので、
  出どころは材質の凹凸で間違いない。
- 直し方の案: 合成 Height にミップを持たせて、グリッドの比に合った LOD で読む。
  または縮小専用のパスを 1 つ挟む。
- 影響は「マスクの細かい模様が、見えているハイトと一致しない」程度。
  大きな形（谷筋に積もる）は正しい。
