...
## 局所ボリュームのシミュレーション記録とループ再生（予定・未着手）

稜線で舞う粉雪のような、局所的で短いループのボリュームアニメーションを作る。ユーザー指定により、EmberGen のような外部ツールの出力（flipbook / VDB）を読むのではなく、**このアプリでシミュレーションを走らせ、その記録を再生する**方向で進める。2026-09-18 に方針だけ決め、実装は未着手。

構成は 3 段階に分ける。

1. **シミュレーション。** 対象のまわりの小さな 3D 格子（例: 200 m 四方 × 100 m を 128 × 128 × 64 セル）で密度と速度を持つオイラー型の簡易な煙ソルバを回す（半ラグランジュ移流、浮力、渦度の補強、地形を障害物として扱う）。箱の外側の速度は下記の「地形全体の風の場」から補間して境界に与え、箱の中は自前の乱流と浮力で細部を作る。発生源は風の場から自動で決める（風下側の稜線で地表の風速が閾値を超える所）。Mask 入力は「ここには出さない／強める」の補正に使う。
2. **記録。** 密度を 3D テクスチャの連番として GPU に持つ。128 × 128 × 64 の 8 bit で 1 フレーム 1 MB、120 フレームで 120 MB。シーンには焼き直せる設定だけを保存し、連番は再生成する運用を基本とする（ディスク保存は DDS 連番か独自のブロック形式を後で検討）。
3. **再生。** 既存の雲のレイマーチに「密度の元」として 3D テクスチャを渡す。照明・雲影・半解像度・時間方向の再投影はそのまま使う。雪向けに位相関数を前方散乱寄りにし、色を白くする程度の違い。

ループは、記録区間を半周期ずらした 2 本を三角波の重みでクロスフェードして継ぎ目を消す。発生源と風が定常なら、落ち着いた区間だけを記録する。

ノードは雲グラフに「Volume Sim」のような 1 ノードを足し、入力に Base（地形）・Mask（発生源）・任意で Path、出力を Cloud Merge へ繋ぐ。プロパティは格子の範囲と解像度、風、浮力、発生量、記録フレーム数とループ長、「焼く」ボタン。再生は雲の再生設定と同じ時間軸に乗せ、ループ位置は Cloud Weather Layer の `loopPosition` と同じ扱いにする。

気にする点:

- 天候層（km 単位）とは格子を別に持つ。同じ画素で重なる所は密度の加算ではなく透過率で合成する必要があり、複数 Volume の合成（[volumetric-clouds.md](../design/volumetric-clouds.md) の未解決事項）に掛かる。まず単独表示を作り、重なりは後回し。
- 焼く時間は 128³ 規模なら 1 フレーム数 ms 台で問題にならないが、UI を止めない非同期にする。
- 見た目は密度の解像度より照明（逆光の透過、細かいちらつき）が効く。局所格子に合わせてレイマーチの刻みを細かくできるようにする。
- 地形を編集すると発生源と障害物が変わるので記録は無効になる。地形の版を持たせ、古い記録には印を出す。

### 地形全体の風の場（2026-09-18 追記）

稜線の粉雪は「谷から吹き上げた風が稜線を越えて風下側で剥がれる」という地形固有の流れなので、代表的な 1 か所の記録を別の稜線へ置き直す（インスタンス配置）方式は使わない。代わりに**風の場を地形全体で 1 つ持ち、局所の格子はそれを境界条件として使う**分担にする。

- 地形全体を覆う粗い 3D 格子（例: 256 × 256 × 32、地表から数百 m まで）で、一様な風を地形にぶつけて発散のない流れに直す。障害物を地形の高さで与え、圧力の投影を数回回す簡易なもの。稜線での吹き上げ、風下の剥離、谷筋への収束という大まかな流れが出ればよい。
- 風向・風速は雲の設定と共有し、変えたら作り直す。地形の版に紐づける。一度作れば静的で、数十 ms・数 MB。
- 粉雪以外（Snow Cover の風、雲の移流を地形に沿わせる）にも使える共通部品として設計する。
- 発生量の分布（風下側の稜線で風速が強い所）を地形に重ねて表示できるようにし、箱を置く目安にする。箱の自動配置（発生量の連結成分から箱を切り、上限を超えたぶんは強い順に採る）はこの分布があって初めて現実的になる。

### 箱の置き方

まず「ノード 1 つ = 箱 1 つ」を手で置く形（中心・幅・奥行き・高さ、Cloud Weather Layer と同じ直方体のガイド）で作る。複数の場所はノードを複数置く。場所が多い運用が見えたら、上記の発生量分布からの自動配置を足す。Mask 入力は最初から持たせ、自動配置に進んでも「箱の中でマスクを読む」部分をそのまま使う。

着手の順番: (1) 局所格子の密度 3D テクスチャを雲のレイマーチで描く（静止 1 フレーム。照明と合成の問題を先に出す）→ (2) 簡易ソルバで動かし、連番を記録して再生、三角波のクロスフェードでループ → (3) 地形全体の風の場を作り、発生量の分布を表示し、局所格子の境界条件と発生源に使う → (4) 保存、焼き直し、雲層との重なり → (5) 箱の自動配置。まず (1) と (2) を一つの区切りとする。規模は雲層の実装と同程度。
