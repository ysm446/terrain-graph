...
IBL天球は地形・マテリアル作成時の作業環境としてルートに保存する。シーンは地形グラフ、大気散乱スカイ、雲グラフをそれぞれ独立参照する。太陽の方位・高度・照度と大気の設定は大気散乱スカイが所有し、雲・地形は共通の太陽を使用する。既存シーンの読み込み互換と保存失敗時の復元を維持する。

## シーン階層と再利用グラフ

ユーザー指定により、シーンから地形・雲グラフを独立アセットとして参照する構成へ移行する。旧シーンを残す明示的な変換、参照の検証、シーン階層、地形・雲の編集切り替えを先行する。現状の描画に合わせて各1組とし、複数配置と部品間の明示的な入出力は後続とする。

## プロジェクトルート方式

ユーザー指定により、シーンに埋め込むアセットをプロジェクトルート配下の独立ファイルへ分離する。初期ルートはdata/。保存・固定ID参照・フォルダブラウザを先行し、検索・監視・ブラウザ内ファイル整理は後続。[設計](../design/project-workspace.md)を参照。
...
モデルアセットはFBX読み込み、メッシュプレビュー、既存LOD選択、マテリアルスロット参照、保存・履歴まで対応。Crumbling Points → Model Scatter → Model Outputによるインスタンス配置にも対応。GPUの視錐台・距離カリングと描画対象の圧縮、間接描画まで対応。ScatterのPointsによる汎用のポイント散布にも対応。Model Mergeで複数の配置を1つのModel Outputへまとめられる。次は遮蔽カリング。自動LOD生成は後回しとする。
