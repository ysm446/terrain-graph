...
1. **シミュレーション。** 対象のまわりの小さな 3D 格子（例: 200 m 四方 × 100 m を 128 × 128 × 64 セル）で密度と速度を持つオイラー型の簡易な煙ソルバを回す（半ラグランジュ移流、浮力、渦度の補強、地形を障害物として扱う）。発生源は地形から取る（Mask Curvature / Mask Slope と標高帯、または Path）。風は雲の風向・風速を元に、地形の傾斜で曲げる。
...
着手の順番: (1) 局所格子の密度 3D テクスチャを雲のレイマーチで描く（静止 1 フレーム。照明と合成の問題を先に出す）→ (2) 簡易ソルバで動かし、連番を記録して再生、三角波のクロスフェードでループ → (3) 発生源を地形マスクから取り、風を地形で曲げる → (4) 保存、焼き直し、雲層との重なり。まず (1) と (2) を一つの区切りとする。規模は雲層の実装と同程度。
