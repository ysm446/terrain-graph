...
ユーザー指定により、手動設定の月・月相・星空と地形・雲への月明かりを追加する。初期対応は昼夜で主光源と影を共有し、月による大気散乱を環境光へ含める。星は星表（BSC5）による実在配置で、観測緯度と恒星時回転で向きを決める。日時からの月と恒星時の計算、昼間の月、太陽と月の同時直射照明は後続。露出は既存の物理カメラ設定を使う。詳細と近似の制約は [大気散乱スカイの設計](../design/atmospheric-sky.md) を参照。雲の位置ごとの拡散環境光は下記の独立した予定として維持する。
