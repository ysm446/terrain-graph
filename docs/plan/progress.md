...
未読み込みアセットは種類名を表示し、開くとプレビューを生成する。外部ファイル変更は「更新」で反映する。検索・ファイル監視・全アセットの自動サムネイル・ブラウザ内の改名/移動/削除は後続。共有アセットの保存はファイル単位で保護し、複数ファイル全体のトランザクションは未対応。[設計](../design/project-workspace.md)を参照。
