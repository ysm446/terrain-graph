...
// weatherCurvature が 0 のときは曲げず、平らな層として扱う（プレビュー設定）。
float WeatherCurvedHeight(float3 position, AtmosphericParameters p) {
    return position.y+p.weatherCurvature*dot(position.xz,position.xz)/(2*kAtmEarthRadius);
}
...
float WeatherCloudDensity(float3 position, AtmosphericParameters p, uint noiseIndex, float viewDistance=0) {
    ...
    float h=(WeatherCurvedHeight(position,p)-p.weatherBottom)/p.weatherThickness;
    if (h<=0 || h>=1) return 0;
    ...
    // 距離 LOD がオフなら刻みは最小のまま、帯域制限と遠景の細部省略も行わない。
    float stepLength=p.weatherLod ? max(p.weatherDetailScale*0.03,viewDistance*0.002*64.0/max(p.samples,16u)) : p.weatherDetailScale*0.03;
    // 移行は最小構造の 1/2 倍から 1.5 倍の刻みにかけて緩やかに行い、切り替わりを目立たせない。
    float fade2=p.weatherLod ? saturate(1.5-stepLength/(p.cloudScale/2.3/16)) : 1;
    ...
    float detailFade=p.weatherLod ? 1-smoothstep(25000,60000,viewDistance) : 1;
    // 刻み幅が細部の最小構造（周期/16）の半分を超えると平均へ寄せ、サンプリングのちらつきを抑える。
    if (p.weatherLod) detailFade*=saturate(1.5-stepLength/(p.weatherDetailScale/16));
...
float4 IntegrateCloudEx(float3 origin, float3 ray, float limit, AtmosphericParameters p,
    ...
    // 多重散乱のオクターブ近似（設計資料 atmospheric-sky.md）。単散乱は維持し、高次は
    // 「広がり」で太陽方向の消散を縮め、寄与は同じ縮小率×反射率で縮める。次数ごとの光路積分は
    // 反射率^n に収まり、単散乱を超える光を作らない。第 5 次は残りの高次を等方位相で代表する。
    const float spread=clamp(p.scatterSpread,0.05,0.9), albedo=saturate(p.indirectLight);
    float phases[5]={CloudPhase(mu,1),CloudPhase(mu,0.5),CloudPhase(mu,0.25),CloudPhase(mu,0.125),0.0795774715};
    float attenuations[5], contributions[5];
    [unroll] for(uint n=0;n<5;++n) {
        attenuations[n]=CloudOrderAttenuation(spread,(float)n);
        contributions[n]=CloudOrderContribution(spread,albedo,(float)n);
    }
    ...
    [loop] for(uint i=0;i<count && transmission>0.005;++i) {
        ...
        if (p.localCloud==4) {
            ...
            float hh=saturate((WeatherCurvedHeight(pos,p)-p.weatherBottom)/(p.weatherThickness*typeTop));
        ...
        [unroll] for(uint order=0;order<5;++order) {
            ...
            light+=sunlight*phases[order]*contributions[order]*exp(-sunDepth*attenuation)*scale
