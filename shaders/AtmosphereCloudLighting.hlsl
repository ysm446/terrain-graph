...
void CsMain(uint3 id:SV_DispatchThreadID) {
    // 惑星地表の Lambert 反射。太陽と月を含む絶対輝度を空・IBL・雲へ共通で渡す。
    ...
    float3 moon=AtmosphereMoon(settings);
    direct+=AtmComputeSunTransmittance(moon,settings.density,settings.mie,0)*max(moon.y,0)*MoonIlluminance(settings);
