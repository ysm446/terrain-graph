# オオシラビソ（大白檜曽、アオモリトドマツ）の簡易モデルを作る Blender スクリプト。
#
# 使い方（Blender 5.x）:
...
#   T_Oshirabiso_Needles_*.png  扁平な針葉が密に付いた小枝のカード（RGBA、A でアルファ抜き）と法線（OpenGL 規約）
#   T_Oshirabiso_Bark_*.png     樹皮（灰色でなめらか。樹脂の膨らみと細かい割れ）と法線
#   Oshirabiso.blend            全バリエーション（手直し用）
...
#
# 形: 亜高山帯の針葉樹林の 1 本。真っすぐな幹 1 本（高さ 9〜16 m）に、ほぼ水平な枝が
# 段ごとに輪生し、細い円錐形の樹冠になる。林の中の木なので下の 2〜3 割は枝が枯れ落ちている。
# 枝の両脇へ水平に小枝を出し、扁平な針葉の付いた小枝のカードを平たく並べる。
...
def make_needle_texture(rng, width=512, height=1024):
    ...
    greens = [(0.07, 0.16, 0.09), (0.06, 0.14, 0.08), (0.09, 0.18, 0.10), (0.08, 0.15, 0.10)]
    cm = 25.6
    ...
    def needles(twig, span, front):
        # 小枝の長さに沿って、1 cm あたり 10 本。左右と前へ開き、先ほど短い。
        # 針葉は小枝の上面を覆うほど密に付く（小枝がほとんど見えない）。
        count = int(span * height * 0.94 / cm * 10)
        for i in range(count):
            ...
            spread = rng.uniform(0.25, 0.95) if not front else rng.uniform(0.0, 0.5)
            direction = normal_dir * spread + ahead * (1.0 - spread * 0.6)
            direction /= max(np.hypot(*direction), 1e-4)
            length = cm * rng.uniform(1.4, 2.6) * (1.0 - 0.3 * s)
            color = np.array(rng.choice(greens)) * rng.uniform(0.85, 1.2)
            # 裏向きの針葉（白い気孔帯が見える）は奥にだけ混ぜる。
            stomata = 0.0 if front else (0.8 if rng.random() < 0.3 else 0.0)
            canvas.stroke(origin, origin + direction * length, 7.5, 5.0, color, stomata)
...
def make_bark_texture(rng, size=512):
    """灰色でなめらかな樹皮。樹脂の膨らみ（丸いこぶ）と、縦の細かい割れ。上下左右に繰り返す。
    画像の x が幹の周、y が幹の長さ（筒の UV と同じ）。"""
    ys, xs = np.mgrid[0:size, 0:size].astype(np.float32)
    tone = tiling_noise(rng, size, 0.03, 0.02)
    color = np.array([0.40, 0.39, 0.36]) + np.array([0.05, 0.05, 0.05]) * np.clip(tone, -2, 2)[..., None]
...
def build_tree(seed, lod):
    ...
    r0 = 0.012 * height + rng.uniform(0.0, 0.04)   # 根元の半径（高さ 12 m で約 17 cm）
    crown_base = rng.uniform(0.2, 0.32)            # 枝の付き始め（高さの割合）
    crown_width = rng.uniform(0.17, 0.22)          # 一番下の枝の長さ（高さの割合）
...
def main():
    ...
    paths = {key: os.path.join(out, f"T_Oshirabiso_{key}.png")
    ...
    bark_mat = os.path.join(out, "MI_Oshirabiso_Bark.tgmat")
    needle_mat = os.path.join(out, "MI_Oshirabiso_Needles.tgmat")
    core_mat = os.path.join(out, "MI_Oshirabiso_Core.tgmat")
    write_material(bark_mat, "MI_Oshirabiso_Bark", source_ref(paths["Bark_D"], root),
                   source_ref(paths["Bark_N"], root), 0.75, 0.0, False)
    write_material(needle_mat, "MI_Oshirabiso_Needles", source_ref(paths["Needles_D"], root),
                   source_ref(paths["Needles_N"], root), 0.6, 0.5, True)
    write_material(core_mat, "MI_Oshirabiso_Core", None, None, 0.9, 0.0, False, CORE_COLOR)

    for index in range(1, options["variants"] + 1):
        name = f"Oshirabiso_Var{index}"
        objects = []
    ...
    bpy.ops.wm.save_as_mainfile(filepath=os.path.join(out, "Oshirabiso.blend"))
