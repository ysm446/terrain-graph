# ハコネダケ（箱根竹）の群落のひとまとまりを作る Blender スクリプト。西天城高原のササ原。
#
# 使い方（Blender 5.x）:
#   blender -b --factory-startup --python-exit-code 1 --python tools/blender/make_hakonedake.py -- #       --out data/Models/Hakonedake [--root data] [--variants 3] [--seed 1]
#
# 出力（--out の下）:
#   Hakonedake_VarN.fbx        稈（Culm）、葉のカード（Leaves）、芯（Core）の 3 スロット。
#                              LOD0〜2 をオブジェクト名の末尾 _LOD<n> で 1 ファイルに入れる
#   T_Hakonedake_Leaves_*.png  葉の付いた小枝のカード（RGBA、A でアルファ抜き）と法線（OpenGL 規約）
#   T_Hakonedake_Culm_*.png    稈（黄緑の筒に節）と法線
#   Hakonedake.blend           全バリエーション（手直し用）
#   *.tgmat / *.tgmodel / *.meta   terrain-graph のアセット。既にあれば上書きしない（UID を保つ）。
#
# 共通の部品は vegetation.py、決まりごとは docs/design/vegetation-assets.md。
#
# 形の作り方はササ（make_sasa.py、伊豆スカイラインの上面が平らな群落）と同じで、違いは次のとおり。
# 西天城高原の写真では、細い葉が上や斜め上へ立って表面がざらざらし、色は鮮やかな黄緑で艶が少ない。
#   - 葉は細い（幅は長さの 12〜17%）。扇は上へ立ち（開きは 22〜40°）、カードも斜めに立てる。
#   - 稈は高さ 1.0〜1.5 m で、上面は平らに揃えずにでこぼこにする。葉は稈の上 5〜6 割に付く。
#   - 色は黄みのある緑（明るさはミヤマハンノキ並み）。艶を抑える（艶があると空を映して白っぽく浮いた）。
import math
import os
import random
import sys
...
def make_leaf_texture(rng, size=512):
    ...
    # 黄緑（西天城の夏の葉）。少しずつ黄み・青みを振る。アルベドは G でリニア 0.15〜0.18 に収める
    # （ミヤマハンノキ並み）。写真の「鮮やかさ」に合わせて 0.35 まで上げたら、同じ光の下で
    # ブナの 3 倍、スギの 7 倍明るくなり、ほかの植生から浮いた。
    greens = [(0.33, 0.47, 0.15), (0.36, 0.50, 0.17), (0.30, 0.44, 0.13), (0.38, 0.52, 0.19)]
    ...
    def fan(origin, count, spread, lengths, lean):
        ...
        for k in range(count):
            ...
            width = length * rng.uniform(0.12, 0.17)
            jitter = np.array([rng.uniform(-5, 5), rng.uniform(-6, 6)], np.float32)
            result.append((origin + jitter, angle, length, width))
        return result

    # 扇は上へ立てる（横へ大きく開くと、上から見て平たい葉の層になる）。
    leaves = fan(top, rng.randint(6, 8), rng.uniform(25, 40), (0.40, 0.50), rng.uniform(-8, 8))
    for _, end, direction in laterals:
        leaves += fan(end, rng.randint(4, 6), rng.uniform(22, 35), (0.32, 0.40), direction * rng.uniform(12, 25))
    rng.shuffle(leaves)
...
LEAF_START = 0.45                 # 稈のどこから上に葉を付けるか（割合）
...
def add_spray(geo, base, axis, rng, scale, cards):
    """葉の小枝 1 つ。1 枚目も寝かせずに軸まわりに傾け、2 枚目はさらに回した面（葉が立って見える）。"""
    ...
    roll = rng.choice((-1, 1)) * math.radians(rng.uniform(35, 60))
...
def build_patch(seed, lod):
    ...
    for _ in range(culms):
        ...
        edge = r / PATCH_RADIUS
        height = rng.uniform(1.0, 1.5) * (1 - 0.06 * edge * edge)
...
def add_foliage(geo, points, seed, outward, lod):
    ...
    for position, tangent, _ in along_polyline(points, LEAF_START, LEAF_SPACING * lod["spacing"], rng):
        ...
        axis = tangent * 0.35 + sideways * rng.uniform(0.4, 0.7) + UP * rng.uniform(0.6, 1.1)
...
CANOPY_NORMAL_WEIGHT = 0.6   # ササより弱く寄せ、葉ごとの明暗を残す（表面のざらつき）
CORE_RADIUS_SCALE = 0.6   # ササより小さく深く（カードを立てたので、ササと同じ大きさでは葉の間から突き出した）
CORE_DROP = 0.35          # 芯を葉の層より下へ沈める量（m）
...
def main():
    ...
    paths = {key: os.path.join(out, f"T_Hakonedake_{key}.png")
    ...
    culm_mat = os.path.join(out, "MI_Hakonedake_Culm.tgmat")
    leaf_mat = os.path.join(out, "MI_Hakonedake_Leaves.tgmat")
    core_mat = os.path.join(out, "MI_Hakonedake_Core.tgmat")
    write_material(culm_mat, "MI_Hakonedake_Culm", source_ref(paths["Culm_D"], root),
                   source_ref(paths["Culm_N"], root), 0.6, 0.0, False)
    # 艶は控えめ（ササの 0.45 では空を映して白っぽく浮いた）。
    write_material(leaf_mat, "MI_Hakonedake_Leaves", source_ref(paths["Leaves_D"], root),
                   source_ref(paths["Leaves_N"], root), 0.62, 0.5, True)
    write_material(core_mat, "MI_Hakonedake_Core", None, None, 0.9, 0.0, False, CORE_COLOR)

    for index in range(1, options["variants"] + 1):
        name = f"Hakonedake_Var{index}"
        objects = []
    ...
    bpy.ops.wm.save_as_mainfile(filepath=os.path.join(out, "Hakonedake.blend"))
