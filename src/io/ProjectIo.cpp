...
void ReadMaterialBody(const json& node, compositor::MaterialAsset& asset,
    ...
    asset.brightness = ReadFloat(node, "brightness", defaults.brightness);
...
bool ReadGraph(const json& node, graph::NodeGraph& graphData, const TextureReader& readTexture,
    ...
    if (const json* items = FindMember(node, "nodes"); items != nullptr && items->is_array()) {
        for (const json& item : *items) {
            ...
            const std::string kindName = ReadString(item, "kind");
            const graph::NodeDefinition* definition = graph::FindNodeDefinitionByName(kindName);
            const int id = ReadInt(item, "id", 0);
            if (id <= 0) {
                continue;
            }
            if (definition == nullptr || definition->kind == graph::NodeKind::Missing) {
                // 知らない種類（廃止した種類、または将来のビルドで増えた種類）は捨てずに
                // ピンのないエラー表示のノードとして残す。リンクは繋がらないまま消える。
                definition = graph::FindNodeDefinition(graph::NodeKind::Missing);
            ...
            // Cloud Merge / Model Merge の可変入力をすべて復元してからリンクを解決する。
            // 旧形式の固定A/Bピンも同じIDで引き継ぐ。
            if ((created.kind==graph::NodeKind::CloudMerge || created.kind==graph::NodeKind::ModelMerge) && inputIds && inputIds->is_array()) {
                for (size_t i=created.inputs.size();i<inputIds->size();++i) {
                    graph::Pin pin;
                    pin.nodeId=created.id; pin.kind=graph::PinKind::Input;
                    pin.valueType=created.kind==graph::NodeKind::ModelMerge ? graph::ValueType::Instances : graph::ValueType::CloudShape;
            ...
            if (created.kind == graph::NodeKind::ModelScatter) {
                ...
                if (const auto* value=FindMember(item,"cloudWeather"); value && value->is_object()) {
                    ...
                    settings.distanceLod=ReadBool(*value,"distanceLod",settings.distanceLod);
                    settings.farDistance=std::clamp(ReadFloat(*value,"farDistance",settings.farDistance),0.0f,200000.0f);
...
json WritePreview(renderer::PreviewRenderer& renderer) {
    ...
    atmosphereNode["nightEnabled"] = atmosphere.nightEnabled != 0;
    atmosphereNode["moonAzimuth"] = atmosphere.moonAzimuth;
    atmosphereNode["moonElevation"] = atmosphere.moonElevation;
    atmosphereNode["moonIlluminance"] = atmosphere.moonIlluminance;
    atmosphereNode["moonPhase"] = atmosphere.moonPhase;
...
void ReadPreview(const json& node, renderer::PreviewRenderer& renderer) {
    ...
    renderer.AtmosphericMode() = ReadString(node, "lightingMode", "ibl") == "atmospheric";
    {
        ...
        atmosphere.nightEnabled = ReadBool(source, "nightEnabled", false) ? 1u : 0u;
        atmosphere.moonAzimuth = std::clamp(ReadFloat(source, "moonAzimuth", defaults.moonAzimuth), -3.1415927f, 3.1415927f);
