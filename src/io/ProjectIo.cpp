    ...
    for (const graph::Node& node : graphData.Nodes()) {
        ...
        if (const auto* settings = std::get_if<graph::LayerNodeSettings>(&node.settings)) {
            ...
            item["cloudWeather"]={{"centerX",weather->centerX},{"centerZ",weather->centerZ},
                ...
                {"noiseScale",weather->noiseScale},{"detailScale",weather->detailScale},{"distanceLod",weather->distanceLod},{"noiseType",EnumName(kCloudNoiseNames,static_cast<uint32_t>(weather->noiseType))},
