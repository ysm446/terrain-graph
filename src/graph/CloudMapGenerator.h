...
inline CompiledCloud GenerateCloudMap(const CloudMapSettings& settings,GraphId nodeId) {
    ...
    const float halfThickness=std::clamp(settings.bottomThickness,2.0f,2000.0f)*0.5f;
    ...
    for (int i=0;i<count;++i) {
        ...
        guide->points.push_back({settings.centerX+x-width*0.5f,baseY,settings.centerZ+z-depth*0.5f});
        ...
        for (int dz=-1;dz<=1;++dz) for (int dx=-1;dx<=1;++dx) {
            ...
            for (const uint32_t j:found->second) {
                ...
                for (int column=0;column<columns;++column) {
                    ...
                    const float height=low+(high-low)*random();
