...
int Application::Run() {
    ...
    while (m_window.PumpMessages()) {
        ...
        if (compiledCloud.hasOutput) {
            ...
            cloudSettings.weatherBottom = compiledCloud.weatherBottom;
            cloudSettings.weatherThickness = compiledCloud.weatherThickness;
            cloudSettings.weatherFar = compiledCloud.farDistance;
...
void Application::BuildDefaultLayout(ImGuiID dockspaceId) {
    ...
    // 前面のタブは右カラムが「グラフ」。アセットの帯は先に描く「テクスチャ」。
