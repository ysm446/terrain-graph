...
- **地形スケール**: プレビューの**平面のサイズ（m）**を 0.5m 〜 8192m で変えられる。
  素材のプレビュー（2m 角）から実際の地形（2km 角）まで扱え、
  カメラの引ける距離・影の範囲・変位量の上限がそれに追従する。
  **メートルなのはジオメトリだけ**で、テクスチャの UV は無次元のまま
  （1 UV が何 m かは決めない）。
- **ハイトマップの取り込み**: 「ハイトマップ」ノード（入力を持たないソース）に
  画像を割り当てると、そのまま地形の形になる。標高差は変位量（m）で決める。
...
| 操作 | 動作 |
| --- | --- |
| 雲トランスフォーム選択中に軸を左ドラッグ / Esc | 雲形状を移動 / ドラッグを取り消す |
...
- **グラフ**（右・前面タブ）— ノードグラフのエディタ。
  ...
  中ドラッグでパン / A で全体を画面に収める）、下段が選択中ノードの設定
